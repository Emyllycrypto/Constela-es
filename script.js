...
const CONFIG = {
  ...
  memories: [
    { date: "—", title: "Quando começamos a namorar", desc: "10 de julho de 2026 - João Pessoa", photo: "imagens/10 de julho.png" },
    { date: "—", title: "Primeiro \"eu te amo\"", desc: "12 de junho de 2026 - Recife", photo: "imagens/12 de junho.png" },
    { date: "—", title: "Primeiro encontro", desc: "27 de abril de 2026 - Jaboatão dos Guararapes", photo: "imagens/27 de abril .png" },
    { date: "—", title: "Primeira viagem juntas", desc: "17 de abril de 2026 - Porto de Galinhas", photo: "imagens/17 de abril.png" }
  ],
  ...
  // Este array só é usado se a seção de planetas voltar a usar
  // <div id="planet-legend"></div> vazio no HTML (renderização automática).
  // Como os planetas agora estão escritos à mão no index.html, ele fica
  // aqui apenas de referência e não é obrigatório manter atualizado.
  planets: [
    { emoji: "🌸", title: "Seu sorriso", desc: "Consegue mudar o meu dia inteiro em um segundo." },
    { emoji: "💫", title: "Seu jeito", desc: "De ser exatamente você, sem pedir desculpas por isso." },
    { emoji: "🤍", title: "Seu carinho", desc: "Que me faz sentir em casa em qualquer lugar." },
    { emoji: "🎶", title: "Sua voz", desc: "A primeira coisa que eu quero ouvir todos os dias." }
  ]
};
...
(function planets(){
  ...
  const colors = ['#ff8fb3', '#f3d98b', '#8d6fd1', '#7fd6c9'];
  const specs = [
    { r: 55, dur: 14 },
    { r: 85, dur: 20 },
    { r: 115, dur: 27 },
  ...
  // Só preenche a legenda automaticamente se existir um contêiner vazio
  // com id="planet-legend". Se os planetas já estiverem escritos à mão
  // no HTML (como no seu index.html atual), este bloco é ignorado.
  if(legend){
    CONFIG.planets.forEach(p=>{
      const item = document.createElement('div');
      item.className = 'planet-item';
      item.innerHTML = `<div class="pi-emoji">${p.emoji}</div><div class="pi-title"></div><div class="pi-desc"></div>`;
      item.querySelector('.pi-title').textContent = p.title;
      item.querySelector('.pi-desc').textContent = p.desc;
